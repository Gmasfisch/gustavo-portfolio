...
/*   Updated: 2026/02/07 15:15:27 by gfischba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	remaining_data(t_node **list)
{
	t_node	*last_node;
	...
	if (k == 0)
		return (dealloc(list, remainder_buf, buf));
	dealloc(list, remainder_buf, buf);
	return (1);
}

int	dealloc(t_node **list, t_node *remainder_buf, char *buf)
{
	t_node	*tmp;
	...
	*list = NULL;
	if (remainder_buf && remainder_buf->buffer[0])
		*list = remainder_buf;
	else
	{
		*list = NULL;
